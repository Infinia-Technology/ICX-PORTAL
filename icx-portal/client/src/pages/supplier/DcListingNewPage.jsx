...
const STEPS = [
  ...
  'Commercial Terms', 'Site Financials', 'Review & Submit',
];

const COOLING_OPTIONS = ['Air Cooled', 'Liquid Cooling Ready (Rear Door/DLC)', 'Hybrid'];
...
export default function DcListingNewPage() {
  ...
  const [step4, setStep4] = useState({ maxRackDensityKw: '', typicalRackDensityKw: '', coolingMethodology: [], liquidCoolingStatus: '', designPue: '', designWue: '', floorMaxWeight: '', landOwner: '', landOwnershipType: '', leaseYears: '', physicalSecurity: '', dcTiering: '', dcTieringCertified: false, iso27001: false, iso50001: false, soc2: false, otherCertifications: '', powerPermitStatus: '', buildingPermitStatus: '', envPermitStatus: '', fireSuppressionType: '', waterFloodRisk: '', seismicRisk: '' });
  ...
  useEffect(() => {
    ...
    api.get(`/dc-applications/${draftAppId}`)
      .then((r) => {
        const app = r.data;
        setStep1({ companyLegalEntity: app.companyLegalEntity || '', companyOfficeAddress: app.companyOfficeAddress || '', companyCountry: app.companyCountry || '', contactName: app.contactName || '', contactEmail: app.contactEmail || '', contactMobile: app.contactMobile || '', otherDetails: app.otherDetails || '' });
  ...
  const stepDataMap = { 1: step2, 2: step3, 3: step4, 4: step5, 5: step6, 6: step7, 7: step8 };
  const currentStepData = stepDataMap[step] || null;
  const { status: autoSaveStatus } = useAutoSave(
    siteId ? `/dc-applications/${appId}/sites/${siteId}` : null,
    currentStepData,
    { enabled: !!siteId && !!appId && step >= 1 && step <= 7 }
  );
  ...
  const saveStep1 = async () => {
    if (!step1.companyLegalEntity || !step1.contactEmail) {
      addToast({ type: 'error', message: 'Company name and contact email are required' });
      return false;
    }
    try {
      if (!appId) {
  ...
  const next = async () => {
    ...
    else if (step === 7) ok = await saveSite({ ...step8 });
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 3:
        return (
          <div className="grid sm:grid-cols-2 gap-4">
            <Input label="Max Rack Density (kW)" name="maxRackDensityKw" type="number" value={step4.maxRackDensityKw} onChange={f(setStep4)} />
            <Input label="Typical Rack Density (kW)" name="typicalRackDensityKw" type="number" value={step4.typicalRackDensityKw} onChange={f(setStep4)} />
            <div className="sm:col-span-2">
            ...
            <Select label="Liquid Cooling Status" name="liquidCoolingStatus" value={step4.liquidCoolingStatus} onChange={f(setStep4)} options={['', 'Installed', 'Ready for Retrofit', 'Design-ready only', 'No'].map((o) => ({ value: o, label: o || 'Select...' }))} />
            <Input label="Design PUE" name="designPue" type="number" step="0.01" value={step4.designPue} onChange={f(setStep4)} />
            <Input label="Design WUE" name="designWue" type="number" step="0.01" value={step4.designWue} onChange={f(setStep4)} />
            ...
            <Select label="Fire Suppression" name="fireSuppressionType" value={step4.fireSuppressionType} onChange={f(setStep4)} options={['', 'Inert Gas', 'Water Mist', 'Pre-Action Sprinkler', 'Hybrid', 'Unknown'].map((o) => ({ value: o, label: o || 'Select...' }))} />
            <Select label="Water/Flood Risk" name="waterFloodRisk" value={step4.waterFloodRisk} onChange={f(setStep4)} options={['', 'Low', 'Medium', 'High', 'Unknown'].map((o) => ({ value: o, label: o || 'Select...' }))} />
            ...
            <div className="flex gap-6 sm:col-span-2">
              <Checkbox label="Tiering Certified" name="dcTieringCertified" checked={step4.dcTieringCertified} onChange={f(setStep4)} />
      ...
      case 4:
        return (
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Input label="Grid Voltage (kV)" name="gridVoltageKv" type="number" value={step5.gridVoltageKv} onChange={f(setStep5)} />
            <Select label="Power Redundancy" name="powerRedundancy" value={step5.powerRedundancy} onChange={f(setStep5)} options={['', 'N', 'N+1', '2N', '2N+1', '2(N+1)', 'Shared Redundant'].map((o) => ({ value: o, label: o || 'Select...' }))} />
            ...
            <Input label="UPS Autonomy (min)" name="upsAutonomyMin" type="number" value={step5.upsAutonomyMin} onChange={f(setStep5)} />
            <Input label="Renewable Energy %" name="renewableEnergyPct" type="number" value={step5.renewableEnergyPct} onChange={f(setStep5)} />
            <div className="sm:col-span-2">
              <p className="text-sm font-medium mb-2">Renewable Types</p>
      ...
      case 5:
        return (
          <div className="grid sm:grid-cols-2 gap-4">
            <Checkbox label="Carrier Neutral" name="carrierNeutral" checked={step6.carrierNeutral} onChange={f(setStep6)} />
            <Input label="Carriers On-Net" name="carriersOnNet" type="number" value={step6.carriersOnNet} onChange={f(setStep6)} />
            <Input label="Carriers Available" name="carriersAvailable" value={step6.carriersAvailable} onChange={f(setStep6)} />
            <Checkbox label="Dark Fibre Available" name="darkFibreAvailable" checked={step6.darkFibreAvailable} onChange={f(setStep6)} />
            <TextArea label="MMR Description" name="mmrDescription" value={step6.mmrDescription} onChange={f(setStep6)} className="sm:col-span-2" />
            ...
            <Input label="Distance to IX (km)" name="distanceToIxKm" type="number" value={step6.distanceToIxKm} onChange={f(setStep6)} />
            <Input label="Latency (ms)" name="latencyMs" type="number" value={step6.latencyMs} onChange={f(setStep6)} />
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <dl className="grid sm:grid-cols-2 gap-2 text-sm">
                ...
                <dt className="text-gray-500">Contact Email</dt><dd>{step1.contactEmail}</dd>
              </dl>
            </Card>
            <Card>
