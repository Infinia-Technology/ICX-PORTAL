import { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import api from '../../lib/api';
import Card from '../../components/ui/Card';
import Badge from '../../components/ui/Badge';
import Button from '../../components/ui/Button';
import TextArea from '../../components/ui/TextArea';
import Spinner from '../../components/ui/Spinner';
import { useToast } from '../../components/ui/Toast';

const STATUS_VARIANT = { PENDING: 'default', SUBMITTED: 'info', APPROVED: 'success', REJECTED: 'error', REVISION_REQUESTED: 'warning' };
const Field = ({ label, value }) => (<div><dt className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">{label}</dt><dd className="text-sm">{value || '—'}</dd></div>);

export default function SupplierDetailPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { addToast } = useToast();
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [reason, setReason] = useState('');
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    api.get(`/admin/suppliers/${id}`).then((r) => setData(r.data)).catch(console.error).finally(() => setLoading(false));
  }, [id]);

  const review = async (action) => {
    setSubmitting(true);
    try {
      await api.put(`/admin/suppliers/${id}/kyc`, { action, reason });
      addToast({ type: 'success', message: `KYC ${action.toLowerCase().replace('_', ' ')}` });
      navigate('/admin/suppliers');
    } catch (err) {
      addToast({ type: 'error', message: err.response?.data?.error || 'Action failed' });
    } finally {
      setSubmitting(false);
    }
  };

  if (loading) return <div className="flex justify-center py-20"><Spinner size="lg" /></div>;
  if (!data) return <div className="text-center py-20">Not found</div>;

  const canReview = ['SUBMITTED', 'RESUBMITTED'].includes(data.status) || data.status === 'REVISION_REQUESTED';

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{data.contactEmail}</h1>
          <div className="flex gap-2 mt-1">
            <Badge variant="default">{data.type}</Badge>
            <Badge variant={STATUS_VARIANT[data.status] || 'default'}>{data.status?.replace(/_/g, ' ')}</Badge>
          </div>
        </div>
        <Button variant="ghost" onClick={() => navigate('/admin/suppliers')}>Back</Button>
      </div>

      <Card>
        <h2 className="font-semibold mb-4">KYC Information</h2>
        <dl className="grid sm:grid-cols-3 gap-4">
          <Field label="Vendor Type" value={data.vendorType} />
          <Field label="Mandate Status" value={data.mandateStatus} />
          <Field label="NDA Required" value={data.ndaRequired ? 'Yes' : 'No'} />
          <Field label="NDA Signed" value={data.ndaSigned ? 'Yes' : 'No'} />
          <Field label="Contact Email" value={data.contactEmail} />
          <Field label="Contact Number" value={data.contactNumber} />
          <Field label="Registered" value={new Date(data.createdAt).toLocaleDateString()} />
        </dl>
      </Card>

      {data.users?.length > 0 && (
        <Card>
          <h2 className="font-semibold mb-4">Users</h2>
          <div className="space-y-2">
            {data.users.map((u) => (
              <div key={u._id} className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded">
                <span>{u.email}</span>
                <div className="flex gap-2">
                  <Badge variant="default">{u.role}</Badge>
                  <Badge variant={u.isActive ? 'success' : 'error'}>{u.isActive ? 'Active' : 'Inactive'}</Badge>
                </div>
              </div>
            ))}
          </div>
        </Card>
      )}

      {canReview && (
        <Card>
          <h2 className="font-semibold mb-4">KYC Review Decision</h2>
          <TextArea label="Reason / Comments" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Optional reason..." rows={3} />
          <div className="flex gap-3 mt-4">
            <Button onClick={() => review('APPROVE')} loading={submitting} className="bg-green-600 hover:bg-green-700">Approve</Button>
            <Button onClick={() => review('REQUEST_REVISION')} loading={submitting} variant="ghost" className="border-yellow-400 text-yellow-700">Request Revision</Button>
            <Button onClick={() => review('REJECT')} loading={submitting} variant="ghost" className="border-red-400 text-red-600">Reject</Button>
          </div>
        </Card>
      )}
    </div>
  );
}
