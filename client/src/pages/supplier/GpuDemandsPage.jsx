...
export default function SupplierGpuDemandsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post(`/gpu-demands/${res.data._id}/submit`);
  ...
  const columns = [
    ...
    { key: '_id', label: 'Actions', render: (v) => <Link to={`/supplier/gpu-demands/${v}`}><Button size="sm">View</Button></Link> },
  ];
    ...
    <div>
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] text-sm mt-1">GPU capacity requests from customers</p>
        </div>
